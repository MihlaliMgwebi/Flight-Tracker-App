...
import { IFlight, IFlights } from '../../../src/models/flight';

export const createFlightSummaryAndDetailsContainer$ = new Subject<IFlights>();
createFlightSummaryAndDetailsContainer$.subscribe((allFlights: IFlights) => {
    const flightSummaryAndDetailsContainer = document.getElementById('app-main__flights')

    if (flightSummaryAndDetailsContainer instanceof HTMLDivElement && allFlights) {
        flightSummaryAndDetailsContainer.classList.remove(`app-main__flights--hidden`)
...
// const flightSVGAndTextandUnitsOfDetailProperties: any = {
//     CALLSIGN: {
...
//     for (let flightSVGAndTextandUnitsOfDetailProperty of flightSVGAndTextandUnitsOfDetailProperties) {
//         const [flightDetail, FlightDetailSVGAndTextandUnits] = flightSVGAndTextandUnitsOfDetailProperty;
...
//         }
// })
// const mainElement: HTMLDivElement = document
//     .getElementById("app-main__flights") as HTMLDivElement;
// mainElement.appendChild(flightSummaryAndDetailsContainer);




function createFlightSummaryCollapsibleButton(flight: IFlight): HTMLButtonElement {
    let flightSummaryCollapsibleButton: HTMLButtonElement = document.createElement("button");
    ...
    flightSummaryCollapsibleButton.addEventListener("click", (event: MouseEvent) => {
        if (event.target instanceof HTMLElement) {
            const onClickEvent = event.target;
            onClickEvent.classList.toggle("active");
            onClickEvent.nextElementSibling?.classList.toggle("hide");
